...
const COVER_DIR = path.join(ROOT, 'maimaiDX-CN-songs-database-main', 'cover');

// ── 加载数据 ──────────────────────────────────────
let maidata, songList, aliasData;
try {
  maidata = JSON.parse(fs.readFileSync(path.join(ROOT, 'maimaiDX-CN-songs-database-main', 'maidata.json'), 'utf8'));
  songList = JSON.parse(fs.readFileSync(path.join(ROOT, 'list.json'), 'utf8'));
  aliasData = JSON.parse(fs.readFileSync(path.join(ROOT, 'anname_list.json'), 'utf8'));
} catch (e) {
...
const titleToCover = {};
maidata.forEach(m => {
  if (m.image_file) titleToCover[m.title.toLowerCase()] = m.image_file;
});
...
if (aliasData.aliases) {
  aliasData.aliases.forEach(a => {
    if (a.aliases) {
      a.aliases.forEach(al => { aliasToSongId[al.toLowerCase()] = a.song_id; });
    }
  });
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (coverMatch) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      // 回退：maidata 中查找
      const maidataEntry = maidata.find(m => m.image_file === filename);
      if (maidataEntry) {
        const fp = path.join(COVER_DIR, maidataEntry.image_file);
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('  Aliases: ' + (aliasData.aliases ? aliasData.aliases.length : 0));
